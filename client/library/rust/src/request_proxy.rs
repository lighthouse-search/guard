...
pub async fn proxy_to_guard(request_metadata: RequestMetadata, body: Option<String>) -> reqwest::Response {
    ...
    let request_builder = match request_metadata.method {
        ...
        _ => panic!("Could not match request method"), // Default to GET
    };

    let req = request_builder.send().await.expect("Failed to fetch upstream");
    req
}

pub async fn package_response_body(req_response: reqwest::Response) -> ProxyResponse {
    // if (req_response.status().is_success() == false) {
    //     println!("Failed to forward request to guard: {}", req_response.status());
