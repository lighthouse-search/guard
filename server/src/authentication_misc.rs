...
pub async fn protocol_decision_to_pipeline(required_scopes: Vec<&str>, hostname: GuardedHostname, jar: &indexmap::IndexMap<String, String>, remote_addr: String, headers: &Headers) -> Result<ProtocolDecisionToPipelineOutput, ErrorResponse> {
    ...
    if authentication_metadata_wrapped.is_none() == false { // Check if a Guard authentication metadata was provided by the client, if it wasn't, we should check other supported sources such as headers.Authorization.
        ...
        let auth_metadata: GuardAuthenticationMetadata = authentication_metadata_wrapped.unwrap();
        ...
        if unverified_authentication_method.method_type == "oauth" {
            ...
            return Ok(ProtocolDecisionToPipelineOutput {
                user: Some(oauth.external_user),
                device: None,
                authentication_method: Some(unverified_authentication_method),
                authentication_type: _authentication_type.unwrap()
            });
...
pub async fn get_guard_authentication_metadata(jar: &indexmap::IndexMap<String, String>, _remote_addr: String, hostname: GuardedHostname, headers: &Headers) -> Result<Option<GuardAuthenticationMetadata>, ErrorResponse> {
    ...
    let output = GuardAuthenticationMetadata {
