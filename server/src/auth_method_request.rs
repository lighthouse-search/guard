...
pub async fn request_email(email: String, authentication_method: AuthMethod, request_data: MagiclinkRequestData, remote_addr: SocketAddr, host: GuardedHostname) -> Result<RequestMagiclink, Box<dyn Error>> {
    ...
    let frontend_metadata: FrontendMetadata = CONFIG_VALUE.clone().frontend.and_then(|f| f.metadata).expect("Failed to parse");
