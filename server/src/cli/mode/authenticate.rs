...
pub async fn handle(arguments: HashMap<String, CommandArgument>, modes: Vec<String>) -> Result<(), String> {
...
async fn request(arguments: HashMap<String, CommandArgument>, _modes: Vec<String>) -> Result<(), String> {
    ...
    if user_authentication.is_err() == false {
        ...
        return Err(serde_json::to_string(&CliAuthenticateHandleResponse {
            error: false,
            nonce: nonce.unwrap(),
    ...
    println!("{}", serde_json::to_string(&CliAuthenticateHandleResponse {
        error: false,
        nonce: nonce.unwrap(),
