...
pub async fn device_pipeline(hostname: Guarded_Hostname, jar: &CookieJar<'_>, remote_addr: SocketAddr, headers: &Headers) -> Result<(bool, Option<Guard_user>, Option<Guard_devices>), Box<dyn Error>> {
    ...
    let (device_result) = device_authentication(signed_data.unwrap()).await;
    ...
    return Ok((true, Some(user), Some(device)));
}