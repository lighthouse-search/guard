...
pub async fn protocol_decision_to_pipeline(required_scopes: Vec<&str>, hostname: Guarded_Hostname, jar: &indexmap::IndexMap<String, String>, remote_addr: String, headers: &Headers) -> Result<(bool, Option<Value>, Option<Guard_devices>,  Option<AuthMethod>, Option<Value>), String> {
    ...
    if (headers.headers_map.get("Authorization").is_none() == false) {
        ...
        if (error_to_respond_with.is_none() == false) {
            log::debug!("oauth_pipeline failed");
        ...
        return Err(format!("guard_authentication_metadata.authentication_method.method_type and headers.Authorization are both unspecified.").into());
    }
}

pub async fn get_guard_authentication_metadata(jar: &indexmap::IndexMap<String, String>, remote_addr: String, hostname: Guarded_Hostname, headers: &Headers) -> (Option<Guard_authentication_metadata>, Option<Value>) {
    ...
    if (auth_metadata.authentication_method.is_none()) {
        ...
        return ((None, Some(error_message("guard_authentication_metadata.authentication_method is null."))));
    }
