use rocket::futures::future::OkInto;
...
pub fn generate_token_string(scope: Vec<String>) -> Token_create {
    ...
    let token_internal = Oauth_server_token_internals {
        scope: scope.join(" "),
        random: random
    };
...
pub async fn check_nonce(user_id: &str, nonce: &str) -> Result<(), String> {
    ...
    .bind::<Text, _>(nonce.clone())
    .load::<Bearer_token>(&mut db)
    .expect("Something went wrong querying the DB.");
...
pub async fn create_access_and_refresh_tokens(user_id: &str, application_clientid: &str, nonce: &str, scope: Vec<String>) -> created_access_and_refresh_tokens {
    ...
    return created_access_and_refresh_tokens {
        access_token: access_token,
        refresh_token: refresh_token
    }
}

pub async fn verify(bearer_token: &str, required_scopes: Vec<&str>) -> Result<Verify_bearer_token_output, String> {
    let mut db = crate::DB_POOL.get().expect("Failed to get a connection from the pool.");
    ...
    let token_internals: Oauth_server_token_internals = serde_json::from_str(&bearer_token_base64_decoded_string).expect("Failed to parse token internal json");

    // Let's verify the token's scope.
    let token_scope: Vec<&str> = token_internals.scope.split(" ").collect();
    let scope_verification = verify_scope(required_scopes.clone(), token_scope.clone()).await.expect("Failed to verify scope.");

    return Ok(Verify_bearer_token_output {
...
async fn verify_hash(bearer_token: &str) -> Result<Verify_bearer_token_hash_output, String> {
    ...
    return Ok(Verify_bearer_token_hash_output {
...
async fn verify_scope(required_scopes: Vec<&str>, token_scope: Vec<&str>) -> Result<(), String> {
    ...
    for required_scope in required_scopes {
        ...
        let scope_position = token_scope.iter().position(|s| s.clone() == required_scope);
